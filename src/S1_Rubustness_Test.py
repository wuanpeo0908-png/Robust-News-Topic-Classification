"""
Robustness_S1.py — Robustness Test cho 5 model S1 (VnExpress)
Data: data_ready_for_model.csv (đã clean)
Đặt tại: VS code/
"""
import os, sys
...
SAVE_DIR = os.path.join(root_path, "Results", "Robustness_S1")
os.makedirs(SAVE_DIR, exist_ok=True)

# ── Load S1 data ─────────────────────────────────────────────────
DATA_PATH = r"D:\Phenikaa-Study\kì 2 2025-2026\Xử lý ngôn ngữ\VS code\Processing\data_ready_for_model.csv"
df = pd.read_csv(DATA_PATH)
df['text_clean'] = df['text_clean'].fillna('').astype(str)
df['label']      = df['label'].astype(int)

_, test_df = train_test_split(df, test_size=0.10, random_state=42, stratify=df['label'])
print(f"🚀 Device: {device} | S1 Test set: {len(test_df)} mẫu")
...
def plot_robustness(noise_res, length_res, model_name):
    ...
    plt.suptitle(f"S1 Robustness: {model_name}", fontsize=13, fontweight='bold')
    plt.tight_layout()
    path = os.path.join(SAVE_DIR, f"robustness_S1_{model_name.replace('+','').replace(' ','_')}.png")
...
# ════════════════════════════════════════════════════════════════
MODELS = {
    "KimCNN"          : (KimCNN(VOCAB_SIZE, 128, 100, [3,4,5], NUM_CLASSES, 0.5),          "kimcnn_s1.pt",           "seq"),
    "BiLSTM_Attention": (BiLSTM_Attention(VOCAB_SIZE, 128, 128, NUM_CLASSES, 2, 0.5),      "bilstm_attention_s1.pt", "seq"),
    "RCNN"            : (RCNN(VOCAB_SIZE, 128, 256, NUM_CLASSES, dropout=0.5),             "rcnn_s1.pt",             "seq"),
    "Transformer"     : (TransformerClassifier(VOCAB_SIZE, 128, 8, 256, 2, NUM_CLASSES, 0.3), "transformer_s1.pt",  "seq"),
    "PhoBERT"         : (PhoBertClassifier(output_dim=NUM_CLASSES, dropout=0.1),           "phobert_s1.pt",          "pho"),
}
...
csv_path = os.path.join(root_path, "Results", "robustness_s1_results.csv")
